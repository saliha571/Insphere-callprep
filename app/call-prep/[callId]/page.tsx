        ...
        <div className="flex flex-shrink-0 flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center sm:gap-8 sm:px-6 sm:py-3.5 print:bg-white">
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-1.5">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="flex items-center gap-0.5 rounded-full border border-amber-100 bg-amber-50 px-2.5 py-1">
                {Array.from({ length: 5 }).map((_, i) => (
                  <svg key={i} className={cn("h-3 w-3 flex-shrink-0", i < call.rating ? "text-amber-400" : "text-slate-200")} viewBox="0 0 20 20" fill="currentColor">
                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                  </svg>
                ))}
                <span className="ml-1 text-[11px] font-medium text-amber-700">{call.rating}/5</span>
              </span>
            </div>
